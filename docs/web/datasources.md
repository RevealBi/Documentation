...
revealView.onDataSourcesRequested = (callback) => {
    ...
    const sqlServerDS = new RVSqlServerDataSource();
    sqlServerDS.host = "your-db-host";
    sqlServerDS.database = "your-db-name";
    sqlServerDS.title = "My SQL Server";

    const sqlServerDSI = new RVSqlServerDataSourceItem(sqlServerDS);
    sqlServerDSI.title = "My SQL Server Item";
    sqlServerDSI.table = "TableName";    

    callback(new RevealDataSources([sqlServerDS], [sqlServerDSI], false));
};
```
...
revealView.onDataSourcesRequested = (callback) => {
    ...
    callback(new RevealDataSources([sqlDataSource], [sqlServerDSI], false));
};
```
