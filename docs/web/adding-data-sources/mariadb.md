...
```js
revealView.onDataSourcesRequested = (callback) => {
    const mariadbDS = new Reveal.RVMariaDBDataSource();
    mariadbDS.title = "MariaDB";
    mariadbDS.subtitle = "Data Source";

    callback(new Reveal.RevealDataSources([mariadbDS], [], false));
};
```
...
revealView.onDataSourcesRequested = (callback) => {
    ...
    const mariadbDSI = new Reveal.RVMariaDBDataSourceItem(mariadbDS);
    mariadbDSI.id = "mariadb_sales_data";
    mariadbDSI.title = "My MariaDB Datasource Item";
    mariadbDSI.subtitle = "MariaDB";

    callback(new Reveal.RevealDataSources([mariadbDS], [mariadbDSI], true));
};
```
