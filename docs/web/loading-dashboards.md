...
2 - In the client application, call the `RevealSdkSettings.setBaseUrl` method and pass in your server URL. When debugging, the server URL will be `https://localhost` followed by a port number. For example:
...
3 - Make a call to the `RVDashboard.loadDashboard` method and pass the name of the dashboard file without the .rdash extension. The method returns a Promise that resolves to the dashboard being requested from the server. Once you have received the dashboard, create an instance of the `RevealView` and set the `RevealView.dashboard` property to the dashboard in the response.
