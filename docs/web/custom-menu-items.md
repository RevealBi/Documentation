...
RVDashboard.loadDashboard("Project Management").then(dashboard => {
    const revealView = new RevealView("#revealView");
    ...
    revealView.onMenuOpening = function (visualization, args) {
        ...
        if (args.menuLocation === RVMenuLocation.Visualization) {
            //adding a new menu item to the "Tasks completed" visualization
            if(visualization.title === "Tasks Completed") {
                const menuItem = new RVMenuItem("Custom Item on Tasks Completed", new RVImage("https://i.pinimg.com/736x/03/c8/a2/03c8a2aff8be6bee9064eef9b5d72d66.jpg", "Icon"), () => {
                    alert('my action');
                })
...
RVDashboard.loadDashboard("Project Management").then(dashboard => {
    ...
    revealView.onMenuOpening = function (visualization, args) {
        // to modify menu behavior on visualizations
        if (args.menuLocation === RVMenuLocation.Visualization) {
            args.menuItems.push(new RVMenuSeparatorItem());
            const menuItem1 = new RVMenuItem("New section item 1", new RVImage("https://i.pinimg.com/736x/03/c8/a2/03c8a2aff8be6bee9064eef9b5d72d66.jpg", "Icon"), () => {
                alert('action 1');
            })
            args.menuItems.push(menuItem1);
            const menuItem2 = new RVMenuItem("New section item 2", new RVImage("https://i.pinimg.com/736x/03/c8/a2/03c8a2aff8be6bee9064eef9b5d72d66.jpg", "Icon"), () => {
                alert('action 2');
            })
