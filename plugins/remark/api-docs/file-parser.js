import { parseJsDocs } from './jsdoc-parser';
import * as parser from '@babel/parser';
import traverse from '@babel/traverse';
import * as t from '@babel/types';
import generate from '@babel/generator';
const fetchFileContents = async (options) => {
    try {
...
const hasPropertyDecorator = (decorators) => {
    if (!decorators)
        return false;
...
const isPublicMethod = (node) => {
    ...
    const isOverride = node.decorators?.some(decorator => t.isCallExpression(decorator.expression) &&
...
const parsePropertyDetails = (node, type) => {
...
const parseAccessorProperty = (node) => {
    ...
    if (node.kind === 'get') {
        ...
        type = extractTypeAnnotation(node.params[0].typeAnnotation);
    }
    return parsePropertyDetails(node, type);
};
const parseMethod = (node) => {
    const name = t.isIdentifier(node.key) ? node.key.name : 'Not Found';
...
export const parseComponentFile = async (options) => {
