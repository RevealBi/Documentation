...
注釈ダイアログには、ダッシュボードの現在のテーマから取得された色見本の行が表示されます。選択した色は注釈の**背景**となり、ラベルとカードを塗りつぶします。固定位置に描画される個別のマーカーではありません。

色の選択は省略可能です。色を割り当てなかった注釈は、折りたたまれた状態では透明になり、マウス ポインターを合わせるか展開すると、テーマの背景色が使用されます。注釈をひと目で認識できるようにする場合は、色を選択してください。
