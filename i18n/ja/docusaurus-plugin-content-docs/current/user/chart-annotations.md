...
表示形式がサポートされているタイプではない場合、表示形式エディターの **[注釈]** ボタンは表示されません。ツール バーにもオーバーフロー メニューにも表示されません。
...
4. **[注釈の追加]** ダイアログが開きます。**[タイトル]** を入力し、必要に応じて **[説明]** と**色**を入力します。
...
注釈が付いたチャートには、各注釈の固定位置にラベルが表示されます。注釈にマウス ポインターを合わせるかタップすると、カードが展開され、注釈のタイトルと説明が表示されます。
...
編集と削除は、**注釈**モードがオンの間のみ使用できます。モードがオンの状態で注釈にマウス ポインターを合わせるかタップするとカードが展開され、アクション ピルに**編集**ボタンと**削除**ボタンが表示されます。通常のダッシュボード表示モードでは、カードにタイトルと説明は表示されますが、これらのボタンは表示されません。

* **[編集]** を選択すると、保存されているタイトル、説明、色を含む注釈ダイアログが再度開きます。必要な箇所を変更し、**[保存]** をクリックまたはタップします。

* **[削除]** を選択すると、注釈が削除されます。元に戻す操作はできません。削除方法によって動作が異なります。カードから削除する場合、Reveal は最初に確認を求めます。**[注釈の編集]** ダイアログから削除する場合は、確認なしで直ちに削除されます。

## 注釈の色の選択

注釈ダイアログには、ダッシュボードの現在のテーマから取得された色見本の行が表示されます。行の先頭には既定の **[色なし]** の色見本があり、末尾にはフル カラー ピッカーを開く **[+]** の色見本があるため、テーマの色以外のカスタム カラーも選択できます。選択した色は注釈の**背景**となり、ラベルとカードを塗りつぶします。固定位置に描画される個別のマーカーではありません。

色の選択は省略可能です。**[色なし]** の色見本を選択した注釈は、折りたたまれた状態では透明になり、マウス ポインターを合わせるか展開すると、テーマの背景色が使用されます。注釈をひと目で認識できるようにする場合は、色を選択してください。

選択した色はテーマを変更しても維持されます。テキストの色、境界線、間隔などその他の外観はすべて有効なテーマから取得されるため、ダッシュボードのスタイルを変更すると注釈もそれに追従します。
