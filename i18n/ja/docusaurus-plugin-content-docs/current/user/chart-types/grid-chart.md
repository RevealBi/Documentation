...
ヘッダーをクリックすると、昇順、降順、並べ替えなしの 3 つの状態が順に切り替わるため、3 回目のクリックで並べ替えが解除されます。**⋮** メニューを開いて、選択されている並べ替えオプションをクリアすることもできます。
...
既定では、列は利用可能な幅を比例配分して共有します。ビジュアライゼーション エディターで明示的に幅を設定した列は、その幅が維持されます。そのため、幅を設定した列を含むグリッドはビジュアライゼーションの幅いっぱいに広がらず、水平方向にスクロールする場合があります。
...
値は画面に表示されているとおりに**書式設定された状態**でコピーされます。通貨セルは `50000` ではなく `$50,000.00` としてコピーされます。

値は一般的な CSV の規則に従って引用符で囲まれるため、区切り文字が曖昧になることはありません。値にカンマ、二重引用符、または改行が含まれる場合、その値は二重引用符で囲まれ、値の内部にある二重引用符は 2 つに重ねられます。たとえば、`6" pipe` というセルは `"6"" pipe"` としてコピーされます。
