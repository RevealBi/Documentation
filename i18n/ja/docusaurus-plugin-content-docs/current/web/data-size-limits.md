...
この設定はデータ読み込み時にサーバー全体に対して一度だけ適用されます。ダッシュボードごとやエクスポートごとのオプションではありません。値を大きくするとサーバー上のすべてのダッシュボードに影響します。また、グリッドおよびピボットのセルは幅が固定されているため、長い値は複数行に折り返され、PDF エクスポートでは行の高さとページ数が増加する点にもご留意ください。
