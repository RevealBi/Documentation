...
グリッド ハイパーリンク列は、グリッド、ピボット、スパークライン、データ グリッドの列の値をクリック可能なリンクに変換します。リンク先と表示テキストの両方にフィールド トークンを含めることができるため、各行のデータに基づいて行ごとに固有のリンク先が生成されます。たとえば、**注文 ID** 列では、各セルがその注文の詳細ページを開きます。
