...
グリッドおよびピボットの PDF エクスポートに含まれる文字列値は `MaxStringCellSize` によって制限されます。デフォルトは 256 文字で、これを超える値は切り捨てられます。「無制限」のオプションはありません。

グリッドおよびピボットのセルは幅が固定されているため、エクスポートされるドキュメントのレイアウトが長い値に合わせて調整されることはありません。長い値は複数行に折り返され、行の高さとページ数が増加します。SDK が値をページに収まるように短縮することはありません。

:::warning
`MaxStringCellSize` はエクスポートの設定ではありません。エンジンはデータをキャッシュに読み込む時点で、つまり表示形式の描画やエクスポートが実行される前にこの設定を適用します。また、これはサーバー全体で 1 つの値であり、ダッシュボードごとやエクスポートごとに上書きすることはできません。PDF の列幅を広げるためにこの値を大きくすると、サーバー上のすべてのダッシュボードについてデータの取り込みとキャッシュの方法も変わります。さらに、変更が反映されるのはその後にキャッシュされるデータのみのため、データ キャッシュのクリアもあわせて必要です。変更する前に[データ制限](https://help.revealbi.io/ja/web/data-size-limits/#maxstringcellsize-と-pdf-エクスポート)をご確認ください。
:::
