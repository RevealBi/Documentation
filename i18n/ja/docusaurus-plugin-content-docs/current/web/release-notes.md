...
#### すべてのプラットフォーム
- グリッドおよびピボットの PDF エクスポートで、文字列値が 200 文字で切り捨てられることがなくなりました。従来、PDF エクスポーターはエンジンの制限に加えて独自の 200 文字の制限を適用していたため、エクスポートに表示されるテキストがダッシュボードよりも短くなる場合がありました。この追加の制限が削除され、エクスポートされるテキストはエンジンの [`MaxStringCellSize`](https://help.revealbi.io/ja/web/data-size-limits/#maxstringcellsize-と長いテキスト値) 設定 (デフォルトは 256 文字) に従うようになり、エクスポートにはダッシュボードと同じ長さのテキストがセルごとに表示されます。さらに多くの文字を表示するには `MaxStringCellSize` を大きくしてください。セルは幅が固定されているため、長い値は折り返されてページ数が増加することにご注意ください。

## 2.2.0 (September 3rd, 2026)

### 重大な変更

- `newTooltip` ベータ機能フラグが削除されました。ホバー ツールチップがデフォルトになったため、このフラグを有効化する呼び出しは削除できます。
- 新しい DataGrid が Reveal SDK のデフォルトのグリッド表示形式になりました。既存のグリッド表示形式は、ダッシュボードを変更しなくても新しい DataGrid で描画されます。従来のグリッドを引き続き使用する場合は、`newDataGrid` 機能フラグを無効にしてください。同じセルに複数の条件付き書式ルールが重なる場合、新しい DataGrid は一致するすべてのルールをリストの順序で適用し、重複する書式プロパティについては後のルールが優先されます。この動作は従来のグリッドとは異なるため、重複するルールに依存しているダッシュボードは、アップグレード後に影響を受ける可能性があります。

### 新機能

- チャートでユーザーが作成した[注釈](https://help.revealbi.io/user/chart-annotations/)がサポートされるようになり、専用のエディターとスタイル設定オプションが追加されました。カテゴリ チャート ファミリーでポイント、スライス、ストリップの注釈タイプを使用できます。注釈はダッシュボードとともに保存されるため、すべての閲覧者が同じ位置で同じメモを確認できます。
- グリッド、ピボット、スパークライン、および新しい DataGrid 表示形式で[ハイパーリンク列](https://help.revealbi.io/web/hyperlink-columns/)がサポートされるようになりました。URL リンクとダッシュボード リンクに対応し、フィールド トークンの置換が可能です。ダッシュボード リンクのコールバックにはセルおよび行のコンテキストが渡されます。
- 再設計された[ツールチップ](https://help.revealbi.io/web/tooltips/)がベータ版ではなくなり、デフォルトになりました。ツールチップは、サポートされるすべての表示形式でホバー時に表示され、ドリル ダウンやフィルタリングなどのツールチップ アクションをツールチップから直接利用できます。表示のオン / オフは `RevealView.showTooltips` で切り替えます。さらに、ツールチップの動作に関する残りの既知の問題も解決し、すべての表示形式でより一貫性のある体験を提供します。
...
- MongoDB コネクタ: スキーマ推論で検出されなかったフィールドを明示的に使用できるようになりました。
- MongoDB のクエリ パフォーマンスと結合処理が改善されました。
- SQL データ ソースのテーブル、ビュー、ストアド プロシージャがアルファベット順に並べ替えられるようになりました。
- 無効なデータ ソース URL のエラー メッセージがより分かりやすくなりました。
- 一般的な日本語のリソース文字列が一貫性のために見直され、修正されました。

### バグ修正

#### すべてのプラットフォーム
- XMLA データ ソースをキャプションで並べ替えたときに発生していたクラッシュを修正しました。
- ページングされたグリッドの Excel および CSV エクスポートで古いデータが出力される問題を修正しました。
- データ フィルター ダイアログを繰り返し閉じたときに発生していたクラッシュを修正しました。
- SQLite コネクタの `SortInterval` をエンジンの実装に合わせて修正しました。
- クエリに `ORDER BY` 句がない場合に MSSQL のページングが失敗する問題を修正しました。
- デフォルトの書式設定を使用する計算フィールドの Excel データ ラベルの問題を修正しました。
- SSRS のいくつかの問題を修正しました。
- ツールチップがフィールドの桁区切り記号の設定に従うようになりました。
- 結合ノードのないサブクエリの処理を修正しました。
- データ フィルターの値リストを更新すると、値が再取得されるようになりました。
- データ ブレンディング使用時のセクション フィールドの問題を修正しました。
- ラジアル チャートが空白で表示される問題を修正しました。
- `Find` 関数が誤った結果を返す問題を修正しました。
- KPI 表示形式の問題を修正しました。
- グリッド ページングのバグを修正しました。
- DataGrid の太字の条件付き書式で斜体のみが適用されていた問題を修正しました。
- 明示的な日付形式が設定されていない列で、DataGrid の日付および時刻セルが書式設定されていない値にフォールバックする問題を修正しました。
- 設定されていても非表示になっている列集計が、DataGrid に表示されないようになりました。

#### ASP.NET
- 日付フィルターのパラメーターを自動読み込みする際に発生していた `Unexpected UserContext instance type` エラーを修正しました。

## 2.1.0 (July 7th, 2026)

### 重大な変更

- レガシーの `$.ig` および `RevealApi` グローバル オブジェクトが削除されました。

### 新機能

- 新しいデータ ソース: SQLite (カスタム クエリをサポート)。
- 新しいデータ ソース: Cube。
- 新しいデータ ソース: DuckDB (ファイルおよび MotherDuck データベース、カスタム クエリ、ストアド プロシージャ (DuckDB マクロ) をサポート)。
- ほとんどのデータベース コネクタが新しい (v2) コネクタ アーキテクチャに移行され、組み込みの SQL インジェクション保護が追加されました。
- [新しい DataGrid ビジュアライゼーション](https://help.revealbi.io/web/beta-features/#newdatagrid) (`newDataGrid` ベータ機能) は、ページングされたデータ ソースに接続されている場合の並べ替え、グループ化、フィルタリングをサポートするようになり、クラシック グリッドのページング エクスペリエンスに近づきました。
- リニア ゲージ、ラジアル ゲージ、ブレット グラフの表示形式の UI が更新され、ベータ版ではなくなりました。
- ピボット、グリッド、スパークライン表示形式のキーボード ナビゲーションとアクセシビリティがさらに改善され、編集モードでの完全なキーボード ナビゲーション サポートが追加されました。
- SQL Server データ ソース: `ApplicationIntent=ReadOnly` および `MultiSubnetFailover` 接続オプションのサポートが追加されました。
- 日付フィルターの UI が更新されました。すべてのフィールドがすでにフィルタリングされている場合、[フィルターの追加] は非表示になります。
- `additionalHeadersProvider` は非同期実装をサポートするようになりました:
...
- Excel エクスポートで数値の書式設定がサポートされるようになりました。
- MongoDB コネクタ: `allowDiskUse` のサポートが追加されました。
- カスタム表示形式テンプレートが改善されました。
- DataGrid 表示形式に `empty-data` テキストが追加され、ダッシュボードに追加された最初の表示形式に表示される空状態アイコンが更新されました。

#### AI
- AI を使用してメタデータ カタログの自動補完を可能にする、メタデータ エンリッチメントの最初のイテレーションが導入されました。デフォルトでは無効です。
- 複数のカスタム AI プロバイダーへの接続のサポートが追加されました (Node と Java。C# にはすでに存在していました)。
- Anthropic プロバイダーが公式の Anthropic SDK に移行されました。
- ホット リロード / 部分的な再生成、ユーザーごとのコンテキスト サポート、新しいプラガブルなメタデータ ストレージ プロバイダーなど、メタデータ レイヤーが改善されました。
- 組み込みコネクタ向けに `networkTimeout` オプションが公開されました。
- ダッシュボード生成において、特定の名前付き四半期および月の日付範囲処理を修正しました。

### バグ修正

#### すべてのプラットフォーム
- 古いインタラクション データが原因で発生していたツリーマップ表示形式のクラッシュを修正しました。
- ピボット表示形式でツールチップが表示されない問題を修正しました。
- 非表示フィールドおよび計算フィールドに対するフィールドベースの条件付き書式の問題を修正しました。
- グリッド表示形式で「データがありません」メッセージが表示されない問題を修正しました。
- 連続するレベルに同じ値が含まれる場合の階層展開の問題を修正しました。
- コンテキスト メニューを開いてもカスタム表示形式が非表示にならないようになりました。
- `RevealView` がダッシュボードを編集前の状態に戻してしまう問題を修正しました。
- 負のパーセンテージの書式設定の問題を修正しました。
- XMLA からタブラーへの変換中に選択した日付の値が正しく伝播しない問題を修正しました。
- すでにそのフィールドでフィルタリングされている場合、「フィルター条件」コンテキスト メニュー オプションが非表示になるようになりました。
- 非表示の値フィールドでフィルタリングした場合にクエリ結果が正しくない問題を修正しました。
- フィルター値の選択に関するいくつかの問題を修正しました。
- バブル チャートの Excel エクスポートでデータ範囲の問題を修正しました。
- 画像エクスポートで、はぐれたダッシュボードの閉じるボタンが誤って復元される問題を修正しました。
- 接頭辞付き名前空間または絶対リレーションシップ ターゲットを使用する XLSX ファイルの読み込みの問題を修正しました。
- 埋め込みダッシュボードでのフィルターと最大化に関する問題を修正しました。
- 最大化の信頼性を向上させました。

#### Java
- Java で BigQuery データ ソース項目を定義する際にエラーが発生する問題を修正しました。
...
- .NET SDK でのみ利用可能だったキャッシュ設定（`maxDownloadSize`、`maxInMemoryCells` など）が Node SDK でも公開されました。
...
- 条件付き書式ルールで null または空のカラー プロパティを開くとクラッシュしていました。
