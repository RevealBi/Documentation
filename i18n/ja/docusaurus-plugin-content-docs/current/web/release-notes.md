...
## 2.2.0 (September 15th, 2026)

### 重大な変更

- MariaDB パッケージが MySQL コネクタに依存しなくなりました。この推移的依存関係に依存していたプロジェクトでは、MySQL コネクタを明示的に参照する必要があります。
- バンドルされている SQLite ネイティブ ライブラリが `SQLitePCLRaw.bundle_e_sqlite3` 2.1.12 に更新されました。
- `newTooltip` ベータ機能フラグが削除されました。ホバー ツールチップがデフォルトになったため、このフラグを有効化する呼び出しは削除できます。

### 新機能

- 新しいコネクタ: Reddit Ads (広告アカウントのメタデータ、キャンペーン、広告グループ、広告の定義、およびパフォーマンス メトリックをサポート)。
- 新しいコネクタ: Microsoft Ads。
...
- [新しい DataGrid 表示形式](https://help.revealbi.io/web/beta-features/#newdatagrid)で、対象となるセルの「フィルター条件」アクションによるインタラクティブ フィルタリングがサポートされるようになりました。
...
- Snowflake で `applyTimeZone` がサポートされるようになりました。
...
- 再設計された[ツールチップ](https://help.revealbi.io/web/tooltips/)がベータ版ではなくなり、デフォルトになりました。ツールチップは、サポートされるすべての表示形式でホバー時に表示され、ドリル ダウンやフィルタリングなどのツールチップ アクションをツールチップから直接利用できます。表示のオン / オフは `RevealView.showTooltips` で切り替えます。
- 無効なデータ ソース URL のエラー メッセージがより分かりやすくなりました。
- Google Ads のマネージャー アカウントにおけるメトリックの拒否が、エラーではなく警告としてログに記録されるようになりました。
- 日本語のリソース文字列が一貫性のために見直され、修正されました。
...
- LinkedIn Organic コネクタで、アカウントに関連付けられた会社がない場合に固有のエラーが報告されるようになりました。
...
- SEO Insights のダッシュボード フィルターの問題を修正しました。
- ツールチップがフィールドの桁区切り記号の設定に従うようになりました。
- `infragistics.rvcore.js` が壊れる可能性のある TypeScript のキャストを修正しました。
...
- KPI 表示形式の問題を修正しました。
- OData データ ソースのクラッシュと検証の再帰の問題を修正しました。
- ClickHouse コネクタが誤ったレベルでログを記録する問題を修正しました。
- WPF のグリッド ページングの問題を修正しました。
- 不要な「Do not create many of these」メッセージを削除しました。
