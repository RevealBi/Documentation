...
**手順 3** - `RevealView.onDataSourcesRequested` イベント内で `RVInMemoryDataSourceItem` を作成します。
...
次に、イベント ハンドラーで、`RVInMemoryDataSourceItem` オブジェクトの新しいインスタンスを作成し、パラメーターとして一意の名前 / ID を指定します。この ID は、`IRVDataProvider` で、要求しているデータはどのデータ ソースかを示すために使用されます。
...
ここで、`revealView` を初期化し、`RevealView.onDataSourcesRequested` イベントを処理します。この例では、**SalesRecords** の `id` と **SalesRecords** に設定された `title` で新しい `RVInMemoryDataSourceItem` を作成しています。
