...
```js
revealView.onDataSourcesRequested = (callback) => {
    const athenaDS = new Reveal.RVAthenaDataSource();
    athenaDS.title = "My Athena Data Source";
    athenaDS.subtitle = "Amazon Athena";

    callback(new Reveal.RevealDataSources([athenaDS], [], false));
};
```
...
revealView.onDataSourcesRequested = (callback) => {
    ...
    const athenaDSI = new Reveal.RVAthenaDataSourceItem(athenaDS);
    athenaDSI.id = "my-data-source-item";
    athenaDSI.title = "My Athena Data Source Item";
    athenaDSI.subtitle = "Amazon Athena";

    callback(new Reveal.RevealDataSources([athenaDS], [athenaDSI], false));
};
```
