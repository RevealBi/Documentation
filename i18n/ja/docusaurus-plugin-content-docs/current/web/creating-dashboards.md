...
新しいダッシュボードの作成は本当に簡単です。`Reveal.RevealView.Dashboard` プロパティを `Reveal.RVDashboard` オブジェクトの新しいインスタンスに設定する必要があります。
...
次に、JavaScript で、`Reveal.RevealView.Dashboard` プロパティを `Reveal.RVDashboard` オブジェクトの新しいインスタンスに設定します。
```js
...
ご覧のとおり、これにより使用する新しいダッシュボード インスタンスが提供されますが、ダッシュボードで使用するデータ ソースを `Reveal.RevealView` に提供していない限り、エンドユーザーは新しいダッシュボードで表示形式を作成できません。
