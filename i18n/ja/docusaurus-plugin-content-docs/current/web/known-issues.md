...
### 問題
RevealView コンポーネントは、`<body>` 要素の末尾に非表示の `<pre>` タグを追加します。このタグは、レイアウト計算のためにフォント サイズを測定するためにビューアーによって内部的に使用されます。アプリケーションに `<pre>` 要素をターゲットとする CSS ルール (グローバルなタイポグラフィやスペーシングのスタイルなど) が含まれている場合、これらのルールが誤って RevealView の測定要素に適用される可能性があります。これにより、UI に予期しない視覚的なアーティファクトやレイアウトのずれが発生する可能性があります。

### 回避策
これらの不要なスタイルが測定要素に影響を与えないようにするには、グローバル スタイルシートに次の CSS ルールを追加します。
```css
