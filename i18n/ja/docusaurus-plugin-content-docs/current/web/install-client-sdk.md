...
:::caution 本番環境では使用しないでください

Reveal SDK CDN は、プロトタイピング、オンボーディング、デモ、および試用シナリオ**のみ**を対象としています。

本番アプリケーションの場合は、Reveal SDK ライブラリを**セルフホスト化する必要があります**。可用性の保証、バージョン管理、パフォーマンスの最適化、長期的な安定性など、本番ワークロードに必要な保証が提供されないため、本番環境で CDN に依存することは推奨されません。

本番環境にデプロイする際は、予測可能な動作と更新の完全な制御を確保するために、SDK アセットをダウンロードして独自のインフラストラクチャから提供してください。
