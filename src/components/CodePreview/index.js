...
const extractCodeBlocks = (children) => {
    const codeBlocks = {
        [LANGUAGES.HTML]: null,
        [LANGUAGES.JS]: null,
    ...
    React.Children.forEach(children, child => {
        if (typeof child === 'string') {
            ...
            const codeElement = React.Children.toArray(preElement).find((c) => c && c.props && c.props.className);
            if (codeElement) {
...
const htmlTemplate = (sdkVersion, body, script) => `
    <!DOCTYPE html>
    <html>
    <head>
...
const Iframe = ({ srcDoc, height }) => {
    ...
    return (<iframe ref={iframeRef} srcDoc={srcDoc} style={{ width: '100%', height: `${height}px`, border: 'none' }}></iframe>);
};
const CodeSnippet = ({ language, code }) => (<CodeBlock language={language} className="code-block">
        {code}
    </CodeBlock>);
const CodePreview = ({ children, previewHeight = 150, sourceOpen = false }) => {
    const { siteConfig } = useDocusaurusContext();
    const version = useDocsVersionFromPath();
    const sdkVersions = siteConfig.customFields.sdkVersions;
    ...
    const srcDoc = htmlTemplate(sdkVersion, codeBlocks[LANGUAGES.HTML], codeBlocks[LANGUAGES.JS]);
    ...
    return (<div className="interactive-code-preview">
            <div style={{ padding: '5px' }}>
                <Iframe srcDoc={srcDoc} height={previewHeight}/>
            </div>

            {showSource && (<div className="code-snippet">
                    {renderCodeSnippet()}
                </div>)}
            ...
            <div className="code-tabs-container">
                ...
                <div className="tabs">
                    {Object.keys(LANGUAGES).map(lang => (codeBlocks[LANGUAGES[lang]] && (<button key={lang} onClick={() => setCurrentTab(LANGUAGES[lang])} className={currentTab === LANGUAGES[lang] ? 'active' : ''}>
                                {lang.toUpperCase()}
                            </button>)))}
