...
function SparkleIcon() {
    return (<svg className={styles.cardIcon} viewBox="0 0 48 48" fill="none" xmlns="http://www.w3.org/2000/svg">
...
function QuickLinkList({ links }) {
    return (<ul className={styles.quickLinks}>
      {links.map((link, idx) => (<li key={idx} className={styles.quickLink}>
...
function Labels({ items }) {
    return (<div className={styles.labels}>
      {items.map((item, idx) => (<span key={idx} className={styles.label}>{item}</span>))}
    </div>);
}
